...
const totalPopulation = populations.reduce((elem, ind) => elem + ind)
...
// CODE HERE
const myStrongest = monstersInYourPocket.filter(function(mobs){
  return mobs.CP > 200
}
console.log(mobs)
...
const orderTotals = orders.map(function(num){
  return num.price + (nums.price * nums.tax)
}
...
// CODE HERE
const bobsTotal = purchases.filter (Function(total) {
  return total.owner === "Bob"
}) .reduce(function(tax,total) {
  return tax + total.price
}, 0)
